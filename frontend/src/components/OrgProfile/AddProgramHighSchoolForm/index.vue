  ...
  <div class="profile__container">
    ...
    <section class="profile__section">
      ...
      <div class="row">
        ...
        <div class="col-50">
          <Select :array="directions"
                  placeholder="Основные направления деятельности"
                  :pre-selected="fields.directions"
                  @select="fields.directions = $event"/>
        </div>
      </div>
    </section>
      ...
      <div class="row">
        <div class="col-100">
          <TagsCloud :tags="fields.competitions"
                     ...
                     placeholder="Навыки, например, управление финансами"
                     :required="true"
                     :margin="0"/>
        </div>
      </div>
      ...
      <div v-for="(item, key) in $v.fields.learning_options.$each.$iter" :key="key" class="profile__group">
        ...
        <section class="profile__section">
          ...
          <div class="row">
            ...
            <div class="col-50">
              <Select :array="[{name: '', id: 0}, {name: '', id: 1}]"
                      placeholder="Бюджетные места"
                      :pre-selected="item.budget_places.$model"
                      @select="item.budget_places.$model = $event"/>
            </div>
          </div>
    ...
    <section class="profile__section">
      <h5 class="title profile__subtitle">
        Ключевые навыки сформированные в процессе обучения
      </h5>
      ...
      <div class="row">
        <div class="col-100">
          <TagsCloud :tags="fields.result_professions"
                     ...
                     placeholder="Навыки, например, самостоятельность"
                     :required="true"
                     :margin="40"
          />
        </div>
...
export default {
  ...
  computed: {
    ...
    directions: function() {
      return this.$dictionaries.inst_direction;
    },
  },
  ...
  methods: {
    ...
    sendData: function(id) {
      ...
      if (this.fields.org_id.length !== 0) {
        if (id) {
          this.$http.put(`${process.env.VUE_APP_DEFAULT_DEVELOP_HOST}api/lk/institutions/${this.fields.org_id}/curricula/${id}`, {...this.fields})
              .then((response) => {
          ...
          this.$http.post(`${process.env.VUE_APP_DEFAULT_DEVELOP_HOST}api/lk/institutions/${this.fields.org_id}/curricula`, {...this.fields})
              .then((response) => {
    ...
    deleteProgram: function() {
      this.$http.delete(`${process.env.VUE_APP_DEFAULT_DEVELOP_HOST}api/lk/institutions/${this.fields.org_id}/curricula/${this.fields.id}`)
          .then(() => {
            this.$router.push({name: 'ProfileOrgProgramsView'});
          }).catch(() => {});
    },
