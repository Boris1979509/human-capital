...
export default {
  ...
  watch: {
    orgInfo(val) {
      if (val.length !== 0) {
        this.orgId = val[0].id;
        this.event.address =
            (val && val[0] && val[0].contacts && val[0].contacts[0] &&
                val[0].contacts[0].address) ||
            '';
      }
    },
    event: {
  ...
  methods: {
    ...
    addEvent() {
      ...
      this.$http.post(
          `${process.env.VUE_APP_DEFAULT_DEVELOP_HOST}api/lk/institutions/${this.$organization[0].id}/journal/`,
        ...
        if (response.status === 201) {
          ...
          this.$router.push({name: 'ProfileOrgEditJournalEventView', params: {id: response.data.id, isSaved: true}});
